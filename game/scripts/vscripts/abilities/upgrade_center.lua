...
function modifier_upgrade_center_state:GetAuraEntityReject(unit)
...
function modifier_upgrade_center_fountain_aura:OnCreated()
	if IsClient() then return end

	local parent = self:GetParent()

	if parent:IsRealHero() then
		EonSpheres:GivePlayerUpgradeAccess(parent:GetPlayerOwnerID())
	end
end

function modifier_upgrade_center_fountain_aura:OnRemoved()
	if IsClient() then return end

	local parent = self:GetParent()

	if parent:IsRealHero() then
		EonSpheres:RemovePlayerUpgradeAccess(parent:GetPlayerOwnerID())
	end
end
...
function upgrade_economy_1:OnSpellStart()
	...
	for i = REFINERY_LEFT, REFINERY_RIGHT do
		local refinery = Refineries:GetRefinery(caster:GetTeam(), i)
...
function modifier_upgrade_creeps:GetAuraEntityReject(unit)
	return unit:HasModifier("modifier_nexus_state")
end
...
function upgrade_tech_1:OnSpellStart()
	...
	for i = BARRACKS_MID, BARRACKS_RIGHT do
		local barracks = BarracksManager:GetBarracks(caster:GetTeam(), i)
	...
	EonSpheres:GiveTeamTierTwoAccess(caster:GetTeam())
end
...
function upgrade_tech_2:OnSpellStart()
	...
	EonSpheres:GiveTeamTierThreeAccess(caster:GetTeam())
end



upgrade_creeps_3 = class({})

function upgrade_creeps_3:OnSpellStart()
	local caster = self:GetCaster()

	LaneCreeps:ResearchSapper(caster:GetTeam())
end