		...
		if not callback_info[1]then
			...
			if callback_info[2] then
				if not callback_info[2].IsNull or not callback_info[2]:IsNull() then
					_, result = ErrorTracking.Try(callback_info[1], callback_info[2], args_tbl)
				else
					EventDriver:CancelListener(event_name, id)
				end
			else
				_, result = ErrorTracking.Try(callback_info[1], args_tbl)
			end
...
EventDriver:Listen("GameMode:state_changed", function(event)
	print("[Event Driver] state changed", event.state)
end)
