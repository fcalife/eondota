...
function table.deepprint(t, indent, done)
	...
	for k, v in ipairs(l) do
		...
		if v ~= 'FDesc' then
			...
			if type(value) == "table" and not done[value] then
				...
				table.deepprint (value, indent + 2, done)
			elseif type(value) == "userdata" and not done[value] then
				done [value] = true
				print(string.rep ("\t", indent)..tostring(v)..": "..tostring(value))
				table.deepprint ((getmetatable(value) and getmetatable(value).__index) or getmetatable(value), indent + 2, done)
			else
